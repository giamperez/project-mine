...
export default function ModuloPortal({
  ...
  function handleCrearNuevaMalla(abrirDirecto = false) {
    ...
    setProyectos((prev) => [nuevoItem, ...prev]);
    setProyectoActivoId(nuevoId);

    if (abrirDirecto) {
      notificar(`✓ Malla "${nuevoNombre}" creada en blanco. Abriendo taller...`);
      onAbrirTaller(nuevoId);
    } else {
      setEditandoId(nuevoId);
      setNombreEnEdicion(nuevoNombre);
      notificar(`✓ Malla "${nuevoNombre}" agregada en blanco a la lista.`);
    }
  }
  ...
  function handleEliminarMalla(proyecto: ItemProyectoPortal, e: React.MouseEvent) {
    ...
    setProyectos((prev) => {
      const filtrados = prev.filter((item) => item.id !== proyecto.id);
      if (proyectoActivoId === proyecto.id) {
        setProyectoActivoId(filtrados.length > 0 ? filtrados[0].id : "");
      }
      return filtrados;
    });
  ...
  function handleGuardarNombre(id: string) {
    ...
    if (limpio) {
      setProyectos((prev) =>
        prev.map((item) => (item.id === id ? { ...item, nombre: limpio } : item))
      );
    }
    setEditandoId(null);
  }
      ...
      <div className="portal-container">
        ...
        <section className="portal-recientes-section">
          <div className="portal-recientes-header">
            ...
            <button
              type="button"
              className="btn-portal-add-inline"
              onClick={() => handleCrearNuevaMalla(false)}
              title="Agregar nueva malla a la lista"
            >
